...
class HuggingfaceModel(BaseModel):
    ...
    def predict(self, input_data, temperature, return_full=False):
        # input_data 是一个字符串，表示输入的文本。
        # Implement prediction.
        inputs = self.tokenizer(input_data, return_tensors="pt").to("cuda")
        # inputs里面有两个key，input_ids和attention_mask，mask中值为 1 为有效输入，需要关注这些位置，为0，无需关注
        ...
        return sliced_answer, log_likelihoods, last_token_embedding, llm_head_input, down_proj_input
    ...
    def get_p_true(self, input_data):
        ...
        model_output_true = model_output_true[0]
