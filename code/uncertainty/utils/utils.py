...
from myllava.model.builder import load_pretrained_model
...
def get_parser(stages=['generate', 'compute']):
    ...
    if 'generate' in stages:
        ...
        parser.add_argument(
            "--num_samples", type=int, default=10000,  # default=400,
        ...
        parser.add_argument(
            "--num_generations", type=int, default=10, #default=10,
...
def init_model(args):
    ...
    if 'llama' in mn.lower() or 'falcon' in mn or 'mistral' in mn.lower():
        model = HuggingfaceModel(mn, stop_sequences='default',max_new_tokens=args.model_max_new_tokens)
    elif mn == "MOF":
        model = load_pretrained_model("/data/public/models/MoF_Models/MoF_Models/MoF_Models", None, "llava")
    else:
...
def save_csv(df, file):
    """
    保存 pandas DataFrame 为 CSV 文件，并将该文件上传到 WandB
    :param df: 要保存的 pandas DataFrame 对象
    :param file: 保存的文件名
    """
